...
- 原 Adapter 规格把 Provider 选择规则修正为逐项 `CLI > env > error`，并链接 [Provider 环境变量回退设计](./2026-07-11-provider-env-fallback-design.md)，移除“只通过必填 CLI 选择”的旧结论。
...
2. 全仓扫描旧 CLI-only 表述应为零，包括“Provider 只通过必填 CLI 参数选择”“不读取 Provider 环境变量”等与 `CLI > env > error` 冲突的句子。
...
6. `.github` Docker workflow 与本轮需求核对后保持无 diff；全仓 `git diff --check`、现有测试、lint 和类型检查继续通过。
